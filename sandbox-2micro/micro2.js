...
export async function micro2_init(module='micro2', ...args){
  ...
  if (typeof internal_wrapper !== 'undefined') {
    return internal_wrapper(() => {_micro2_init(...args)});
  } else {
    return _micro2_init(module, ...args);
  }
}
  
function _micro2_init(module, mount_func) {
    ...
    module_root.innerHTML = `
      <div class="module_header">
        ...
        <span class="component_sentry_controls">
          <input type="checkbox" id="${module}_sentry_enabled" name="lms"><label for="lms">Sentry</label>
          <input type="checkbox" id="${module}_sentry_tracing" name="mst"><label for="mst">tracing</label>
          <input type="checkbox" id="${module}_sentry_debug" name="msd"><label for="msd">debug</label>
        </span>
      </div>`;
    ...
    Com.sticky_checkbox_init(`${module}_sentry_enabled`,  true);
    Com.sticky_checkbox_init(`${module}_sentry_tracing`,  true);
    Com.sticky_checkbox_init(`${module}_sentry_debug`,    false);
    if (MICRO2_PROJECT_URL) {Com.add_project_link(MICRO2_PROJECT_URL, module, "label[for=msd]");};
